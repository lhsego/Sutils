...
void CalcSchillingInv2(double *pdDefective, int *piTotal, int *piSamples, double *pdConf, double *pdAchievedAccept) // p is for pointer
{
  ...
  int iTotal = *piTotal, iSamples = *piSamples;
	...
	// else if % Acceptable is 1
		...
		dConf = double(iSamples) / double(iTotal);
		dAchievedAccept = 1.0;
	...
	// Otherwise calculate the confidence
		...
		double dN = double(iTotal);
		double dn = double(iSamples);
		double dPd = dDefective / 100.0;
		double dLambda0 = 1.0 - dPd;

		// A tolerance variable for searches and two candiate variables for achieved % Acceptable
		double dTol = 5e-10, dLambda1 = 0.0, dLambda2 = 0.0;
		...
		for (i = 1; i <= iTotal; i++) {
			...
			if (QuickCalc(dn, dN, double(i)) < dTol) {
				dFindFirstU = double(i);
				break;
			}
		...
		if (iDoBinarySearch) {
			...
			while ((fabs(dUlo - dUhi) > 1e-10) & (i < 10000)) {
				...
				if (QuickCalc(dn, dN, dTestU) < dTol) {
					dUlo = dTestU;
				} else {
					dUhi = dTestU;
				}
